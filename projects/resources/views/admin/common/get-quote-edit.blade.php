                ...
                <tr>
                 ...
                 <tr>
                    <th colspan="4">
                        ...
                        @if($c->accessories_type=='per night')
                        <input type="hidden" value="{{ $value_1 }}" name="accessories_rate_id[field][{{$c->id}}]" id="accessories_rate_id{{$c->id}}field" class="{{ in_array($c->id,$ids)?'':'d-none' }}  rate-calculateion-data"> 
                       @else
                      	<input type="text" value="{{ $value_1 }}" name="accessories_rate_id[field][{{$c->id}}]" id="accessories_rate_id{{$c->id}}field" class="{{ in_array($c->id,$ids)?'':'d-none' }}  rate-calculateion-data"> 
                       @endif
...
@endphp
        @foreach($amount_data as $c)
              ...
              @endif
            <tr>
                <td colspan="4"  align="left"><strong>{{$c['message']}} {!! $status !!}</strong></td>
                <td align="right">{!! ModelHelper::getDataFromSetting('payment_currency') !!}{{number_format($c['amount'],2)}}</td>
            </tr>
        @endforeach
      ...
      <input type="hidden" name="discount" value="{{ $discount }}"  id="coupon_discount"  />
