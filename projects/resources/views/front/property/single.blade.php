    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-4 right">
                        ...
                        @php $k=1; @endphp
                	   @foreach(App\Models\PropertyGallery::where("property_id",$data->id)->limit(2)->orderBy("sorting","asc")->get() as $c)
                                     
                      
                         	 <img src="{{asset($c->image)}}" class="img-fluid" alt="{{$c->caption}}" class="img{{ $k++ }}">
                ...
                <div id="gallery-1" class="hidden">
                	   ...
                	   @foreach(App\Models\PropertyGallery::where("property_id",$data->id)->orderBy("sorting","asc")->get() as $c)
                         	...
                         	<a href="{{asset($c->image)}}"  title="{{$c->caption}}"></a>
 ...
 <section id="property" class="padding_top padding_bottom_half">
               ...
               <div class="upper-box">
                   <div class="row">
                       <div class="col-lg-12 col-md-12 col-12 col-sm-12">
                           ...
                           <h3>{{$data->name}}</h3> 
                            <div class="hotel-info"><i class="fas fa-map-marker-alt"></i>{{$data->address}}</div>
                       </div>
                   ...
                   <div class="pro-ammenities">
                        <div class="amn-top1">
                    <p><img src="{{ asset('front')}}/images/rv.png" class="img-fluid"> {{ App\Models\Location::find($data->location_id)->name ?? '' }}</p> 
                    @if($data->address)
                    <p><i class="bi bi-geo-alt-fill"></i> {{$data->address}}</p>
                    @endif
                    @if($data->sleeps)
                    <p><i class="bi bi-person-fill-add"></i> {{$data->sleeps}} adults</p>
                    @endif
                      @if($data->beds)
                    <p><i class="fa-solid fa-gas-pump"></i> {{$data->beds}} Seats</p>
                    @endif
                    </div>
                </div>
               </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="price-side">
            <p><span> {!! $setting_data['payment_currency'] !!}{{Helper::getPriceAmountData($data->id)}}</span> / night</p>
        </div>
...
<section class="related-property">
    <div class="container">
        ...
        <div class="row">
           @foreach($related_list as $c)
            <div class="col-3">
                <a href="{{ url('properties/detail/'.$c->seo_url) }}">
                     <div class="img-section">
                    <img src="{{asset($c->feature_image)}}" class="img-fluid" alt="">
                    <p class="pr">{!! $setting_data['payment_currency'] !!}{{Helper::getPriceAmountData($c->id)}} / Night</p>
                    </div>
                    <div class="content">
                        ...
                        <ul class="property-room-features">
                                    ...
                                    <li><span><img src="{{ asset('front/images/camper-van.png')}}" class="img-fluid"> {{ App\Models\Location::find($c->location_id)->name ?? '' }}</span></li>
                                            @if($c->address)
                                            <li><span><i class="bi bi-geo-alt-fill"></i> {{$c->address}}</span></li>
                                            @endif
                                            @if($c->sleeps)
                                            <li><span><i class="bi bi-person-fill-add"></i> {{$c->sleeps}} adults</span></li>
                                            @endif
                                            @if($c->beds)
                                            <li><span><i class="fa-solid fa-gas-pump"></i> {{ $c->beds }} Seats</span></li>
                                            @endif
                                </ul>
